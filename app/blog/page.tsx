...
type Post = { _id: string; title: string; slug: string; excerpt?: string; date?: string };
    ...
    <main className="min-h-screen">
      <section className="w-full max-w-5xl mx-auto px-4 md:px-8 py-16">
        ...
        <div className="mt-8 grid md:grid-cols-2 gap-6">
          {posts.map(p => (
            <a key={p._id} href={`/blog/${p.slug}`} className="block border rounded-2xl p-6 hover:shadow bg-white">
              <div className="text-xs text-slate-500">{p.date ? new Date(p.date).toLocaleDateString() : ''}</div>
