...
export async function generateMetadata({ params }:{ params:{ slug:string } }){
  ...
  return {
    ...
    openGraph: { title: data.title, description: data.excerpt, url: `/blog/${data.slug}`, images: data.cover ? [{ url: data.cover, width: 1200, height: 630, alt: data.title }] : undefined }
  };
}
  ...
  return (
    <main className="min-h-screen">
      <section className="w-full max-w-3xl mx-auto px-4 md:px-8 py-16 prose prose-slate">
        ...
        <div className="mt-2 text-sm text-slate-500">{new Date(data.date).toLocaleDateString()}</div>
